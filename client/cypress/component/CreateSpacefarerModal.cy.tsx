...
function stubRefData() {
  ...
  cy.intercept('GET', '/api/positions',   { statusCode: 200, body: [{ id: 'P1', title: 'Cadet', rank: 1 }] }).as('positions');
}
...
describe('CreateSpacefarerModal', () => {
  ...
  it('sends a POST to /api/spacefarers with correct payload', () => {
    ...
    page.fill({ name: 'Zara Nova', email: 'zara@galactic.space', planet: 'PlanetX' });
    page.submit();
  ...
  it('calls onCreated after a successful submission', () => {
    cy.intercept('POST', '/api/spacefarers', { statusCode: 201, body: { id: 'S-new', name: 'Zara Nova' } });
    page.fill({ name: 'Zara Nova', email: 'zara@galactic.space', planet: 'PlanetX' });
    page.submit();
...
describe('CreateSpacefarerModal — additional coverage', () => {
  ...
  it('sends updated spacesuit colour when changed', () => {
    cy.intercept('POST', '/api/spacefarers', { statusCode: 201, body: { id: 'S-new' } }).as('req');
    page.fill({ name: 'Zara', email: 'zara@g.space', planet: 'PlanetX' });
    page.colorInput().clear().type('Cyan');
    page.submit();
    cy.wait('@req').its('request.body').should('include', { spacesuitColor: 'Cyan' });
  });
  ...
  it('sends updated stardust collection when changed', () => {
    ...
    page.submit();
    cy.wait('@req').its('request.body').should('include', { stardustCollection: 250 });
  });
  ...
  it('sends updated wormhole skill when changed', () => {
    ...
    page.submit();
    cy.wait('@req').its('request.body').should('include', { wormholeNavigationSkill: 9 });
  });
  ...
  it('sends departmentId when a department is selected', () => {
    ...
    page.submit();
    cy.wait('@req').its('request.body').should('include', { departmentId: 'D1' });
  });
  ...
  it('allows changing the pre-filled origin planet', () => {
    ...
    page.submit();
    cy.wait('@req').its('request.body').should('include', { originPlanet: 'PlanetZ' });
  });
});
