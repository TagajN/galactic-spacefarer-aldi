...
function stubRefData() {
  cy.intercept('GET', '/api/departments', { statusCode: 200, body: [{ id: 'D1', name: 'Stellar Engineering', galaxy: 'Milky Way' }] }).as('depts');
  cy.intercept('GET', '/api/positions',   { statusCode: 200, body: [{ id: 'P3', title: 'Pilot', rank: 3 }] }).as('positions');
}
...
describe('SpacefarerDetail', () => {
  ...
  it('calls PATCH /api/spacefarers/:id with updated values on Save', () => {
    ...
    cy.wait('@patchReq').its('request.body').should('include', { spacesuitColor: 'Gold' });
  ...
  it('updates status via the status dropdown', () => {
    ...
    cy.wait('@patchReq').its('request.body').should('include', { status: 'RETIRED' });
  });

  it('updates department via the department dropdown', () => {
    cy.intercept('PATCH', '/api/spacefarers/S1', {
      statusCode: 200,
      body: { ...SF, department_id: 'D1', department_name: 'Stellar Engineering' },
    }).as('patchReq');
    ...
    cy.wait('@patchReq').its('request.body').should('include', { departmentId: 'D1' });
  });
...
describe('SpacefarerDetail — additional coverage', () => {
  ...
  it('hides Edit button for viewer users', () => {
    ...
    cy.mount(
      <AuthProvider>
        <SpacefarerDetail spacefarer={SF} onBack={cy.stub()} onUpdated={cy.stub()} />
      </AuthProvider>
    );
  ...
  it('sends updated name and email in PATCH request', () => {
    cy.intercept('PATCH', '/api/spacefarers/S1', { statusCode: 200, body: SF }).as('patch');
    mountAsAdmin();
    page.clickEdit();
  ...
  it('sends updated positionId when position is selected', () => {
    ...
    page.save();
    cy.wait('@patch').its('request.body').should('include', { positionId: 'P3' });
  });
  ...
  it('sends undefined departmentId when — None — is selected', () => {
    ...
    page.save();
    cy.wait('@patch').its('request.body').should('not.have.property', 'departmentId');
  });

  it('disables Save button and shows Saving… while request is in flight', () => {
    cy.intercept('PATCH', '/api/spacefarers/S1', { delay: 3000, statusCode: 200, body: SF });
    mountAsAdmin();
    page.clickEdit();
    page.save();
