...
const ALICE: Spacefarer = {
  ...
  origin_planet: 'PlanetX', spacesuit_color: 'Silver',
  ...
  department_name: 'Stellar Engineering', position_title: 'Pilot', position_rank: 3,
  created_at: '2024-01-15T09:00:00Z', modified_at: '2024-01-15T09:00:00Z',
};

const DAVE: Spacefarer = {
  ...ALICE,
  id: 'S4', name: 'Dave Pulsar', email: 'dave@galactic.space',
  spacesuit_color: 'Red', stardust_collection: 150,
  wormhole_navigation_skill: 4, status: 'CANDIDATE',
};
...
function mountAsAdmin() {
  localStorage.setItem('user', JSON.stringify({ id: 'U1', username: 'alice', role: 'admin', planet: 'PlanetX' }));
...
function mountAsViewer() {
  localStorage.setItem('user', JSON.stringify({ id: 'U3', username: 'carol', role: 'viewer', planet: 'PlanetX' }));
  localStorage.setItem('token', 'fake-token');
  cy.mount(
    <AuthProvider>
      <SpacefarerList onSelect={cy.stub().as('onSelect')} />
...
describe('SpacefarerList', () => {
  ...
  it('calls onSelect when a row is clicked', () => {
    ...
    cy.get('@onSelect').should('have.been.calledWith', Cypress.sinon.match({ id: 'S1' }));
  });
  ...
  it('sends status filter as query param when filter changes', () => {
    ...
    cy.wait('@filteredReq').its('request.url').should('include', 'status=CANDIDATE');
  });
  ...
  it('calls DELETE /api/spacefarers/:id and reloads list when Delete is confirmed', () => {
    ...
    cy.intercept('DELETE', '/api/spacefarers/S1', { statusCode: 204, body: null }).as('deleteReq');
  ...
  it('calls PATCH /retire and reloads list', () => {
    ...
    cy.intercept('GET', '/api/spacefarers*', {
      statusCode: 200,
      body: { data: [{ ...ALICE, status: 'RETIRED' }, DAVE], total: 2, page: 1, pageSize: 10 },
    }).as('reloaded');
...
describe('SpacefarerList — additional coverage', () => {
  it('shows error message when API call fails', () => {
    cy.intercept('GET', '/api/spacefarers*', { statusCode: 500, body: { error: 'Server exploded' } }).as('fail');
    mountAsAdmin();
    cy.wait('@fail');
  ...
  it('shows empty-state row when no spacefarers are found', () => {
    cy.intercept('GET', '/api/spacefarers*', { statusCode: 200, body: { data: [], total: 0, page: 1, pageSize: 10 } });
    mountAsAdmin();
    cy.contains('No spacefarers found').should('be.visible');
  });

  it('shows singular "1 spacefarer found" when total is 1', () => {
    cy.intercept('GET', '/api/spacefarers*', { statusCode: 200, body: { data: [ALICE], total: 1, page: 1, pageSize: 10 } });
    mountAsAdmin();
    cy.contains('1 spacefarer found').should('be.visible');
  });

  it('disables Next button when already on the last page', () => {
    cy.intercept('GET', '/api/spacefarers*', { statusCode: 200, body: { data: [ALICE], total: 5, page: 1, pageSize: 10 } });
    mountAsAdmin();
  ...
  it('sends spacesuitColor as query param when filter is typed', () => {
    ...
    cy.wait('@colourReq').its('request.url').should('include', 'spacesuitColor=Silver');
  });
  ...
  it('toggles sort direction back to Asc after being set to Desc', () => {
    ...
    cy.intercept('GET', '/api/spacefarers*sortDir=desc*', { statusCode: 200, body: { data: [DAVE, ALICE], total: 2, page: 1, pageSize: 10 } });
    page.sortDirButton().click();

    cy.intercept('GET', '/api/spacefarers*', { statusCode: 200, body: { data: [ALICE, DAVE], total: 2, page: 1, pageSize: 10 } }).as('ascReq');
    page.sortDirButton().click();
  ...
  it('sends sortBy param when sort field is changed', () => {
    ...
    cy.wait('@sortReq').its('request.url').should('include', 'sortBy=stardust_collection');
  });

  it('shows — when spacefarer has no department', () => {
    const noDept = { ...ALICE, department_id: null, department_name: undefined };
    cy.intercept('GET', '/api/spacefarers*', { statusCode: 200, body: { data: [noDept], total: 1, page: 1, pageSize: 10 } });
    mountAsAdmin();
  ...
  it('shows alert when Delete API call fails', () => {
    ...
    cy.intercept('DELETE', '/api/spacefarers/S1', { statusCode: 500, body: { error: 'DB error' } });
  ...
  it('shows alert when Retire API call fails', () => {
    ...
    cy.intercept('PATCH', '/api/spacefarers/S1/retire', { statusCode: 500, body: { error: 'Retire failed' } });
  ...
  it('hides Retire button for already-RETIRED spacefarers', () => {
    ...
    cy.intercept('GET', '/api/spacefarers*', { statusCode: 200, body: { data: [retired], total: 1, page: 1, pageSize: 10 } });
    mountAsAdmin();
