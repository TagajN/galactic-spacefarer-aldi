...
import { api } from '../api';
import type { Spacefarer, Department, Position, UpdateSpacefarerBody, SpacefarerStatus } from '../types';
...
export default function SpacefarerDetail({ spacefarer: initial, onBack, onUpdated }: Props) {
    ...
    <div className="space-y-6">
      ...
      {!editing ? (
        <div className="space-y-4">
          <h3 className="text-gray-400 text-xs uppercase tracking-widest">Basic Information</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {field('Name',    sf.name)}
            {field('Email',   sf.email)}
            {field('Planet',  sf.origin_planet)}
            {field('Status',  <StatusBadge status={sf.status} />)}
            {field('Launch Date', sf.launch_date ? new Date(sf.launch_date).toLocaleDateString() : null)}
          </div>
          <h3 className="text-gray-400 text-xs uppercase tracking-widest mt-4">Cosmic Skills & Gear</h3>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {field('Stardust Collection',      `⭐ ${sf.stardust_collection}`)}
            {field('Wormhole Navigation Skill',`${sf.wormhole_navigation_skill} / 10`)}
            {field('Spacesuit Colour',         sf.spacesuit_color)}
          </div>

          <h3 className="text-gray-400 text-xs uppercase tracking-widest mt-4">Galactic Assignments</h3>
          <div className="grid grid-cols-2 gap-3">
            {field('Department', sf.department_name)}
            {field('Position',   sf.position_title ? `${sf.position_title} (Rank ${sf.position_rank})` : null)}
          </div>
        </div>
      ) : (
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <label className="block">
              ...
              <input data-cy="field-name" type="text" value={form.name ?? ''} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
            ...
            <label className="block">
              ...
              <input data-cy="field-email" type="email" value={form.email ?? ''} onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                className="mt-1 w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </label>
            <label className="block">
              <span className="text-xs text-gray-400">Spacesuit Colour</span>
              <input data-cy="field-spacesuitColor" type="text" value={form.spacesuitColor ?? ''} onChange={e => setForm(f => ({ ...f, spacesuitColor: e.target.value }))}
                className="mt-1 w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </label>
            <label className="block">
              <span className="text-xs text-gray-400">Stardust Collection</span>
              <input data-cy="field-stardustCollection" type="number" min={0} value={form.stardustCollection ?? 0} onChange={e => setForm(f => ({ ...f, stardustCollection: Number(e.target.value) }))}
                className="mt-1 w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </label>
            <label className="block">
              <span className="text-xs text-gray-400">Wormhole Skill (1–10)</span>
              <input data-cy="field-wormholeSkill" type="number" min={1} max={10} value={form.wormholeNavigationSkill ?? 5} onChange={e => setForm(f => ({ ...f, wormholeNavigationSkill: Number(e.target.value) }))}
                className="mt-1 w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </label>
            <label className="block">
              <span className="text-xs text-gray-400">Status</span>
              <select value={form.status ?? sf.status} onChange={e => setForm(f => ({ ...f, status: e.target.value as SpacefarerStatus }))}
                className="mt-1 w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <label className="block">
              ...
              <select value={form.departmentId ?? ''} onChange={e => setForm(f => ({ ...f, departmentId: e.target.value || undefined }))}
                className="mt-1 w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <label className="block">
              ...
              <select value={form.positionId ?? ''} onChange={e => setForm(f => ({ ...f, positionId: e.target.value || undefined }))}
                className="mt-1 w-full bg-gray-800 border border-gray-600 text-white rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                <option value="">— None —</option>
                {positions.map(p => <option key={p.id} value={p.id}>{p.title} (Rank {p.rank})</option>)}
              </select>
            </label>
          </div>
          {error && <p className="text-red-400 text-sm">{error}</p>}
          <div className="flex gap-3 pt-2">
            <button data-cy="save-btn" onClick={handleSave} disabled={saving}
