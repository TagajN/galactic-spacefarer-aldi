...
export default function SpacefarerList({ onSelect }: Props) {
  ...
  const load = useCallback(async (p = 1) => {
    ...
    setError('');
    try {
      const q: SpacefarersQuery = { page: p, pageSize: PAGE_SIZE, sortBy, sortDir };
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 flex flex-wrap gap-3 items-end">
        ...
        <div>
          <label className="block text-xs text-gray-400 mb-1">Spacesuit Colour</label>
          <input
            type="text"
        ...
        <button
          onClick={() => { setStatus(''); setColor(''); setSortBy('name'); setSortDir('asc'); }}
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-gray-800">
            ...
            {!loading && items.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500">No spacefarers found</td></tr>
            )}
            {!loading && items.map(sf => (
              <tr
                ...
                onClick={() => onSelect(sf)}
              >
                <td className="px-4 py-3 text-white font-medium">{sf.name}</td>
                <td className="px-4 py-3 text-gray-300">{sf.origin_planet}</td>
                <td className="px-4 py-3">
                  <span className="inline-block w-3 h-3 rounded-full mr-2 border border-gray-600"
                    style={{ backgroundColor: sf.spacesuit_color.toLowerCase() }} />
                  <span className="text-gray-300">{sf.spacesuit_color}</span>
                </td>
                <td className="px-4 py-3 text-gray-300">⭐ {sf.stardust_collection}</td>
                <td className="px-4 py-3 text-gray-300">{sf.wormhole_navigation_skill}/10</td>
                <td className="px-4 py-3"><StatusBadge status={sf.status} /></td>
                <td className="px-4 py-3 text-gray-400">{sf.department_name ?? '—'}</td>
      ...
      <div className="flex items-center justify-between text-sm text-gray-400">
        <span>{total} spacefarer{total !== 1 ? 's' : ''} found</span>
        <div className="flex gap-2">
          ...
          <span className="px-3 py-1">Page {page} / {Math.max(1, totalPages)}</span>
          <button
