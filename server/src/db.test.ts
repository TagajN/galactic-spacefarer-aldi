...
beforeEach(() => {
  tmpDb = path.join(os.tmpdir(), `galactic-test-${Date.now()}-${Math.random()}.db`);
  setDbPath(tmpDb);
  migrate();
});
...
describe('listDepartments', () => {
  it('returns all seeded departments ordered by name', () => {
    ...
    expect(depts[0]).toMatchObject({ id: expect.any(String), name: expect.any(String) });
  });
});
...
describe('listPositions', () => {
  it('returns all seeded positions ordered by rank', () => {
    ...
    expect(positions[0].rank).toBeLessThanOrEqual(positions[positions.length - 1].rank);
  });
});
...
describe('createSpacefarer', () => {
  it('inserts a new spacefarer and returns it with joined data', () => {
    const created = createSpacefarer({
      name: 'Zara Nova', email: 'zara@test.space',
      originPlanet: 'PlanetX', spacesuitColor: 'Cyan',
      stardustCollection: 200, wormholeNavigationSkill: 7,
      departmentId: 'D1',
    });
  ...
  it('applies default values when optional fields are omitted', () => {
    const created = createSpacefarer({
      name: 'Min Fields', email: 'min@test.space', originPlanet: 'PlanetY',
    });
...
describe('listSpacefarers', () => {
  it('returns only spacefarers from the requested planet', () => {
    ...
    expect(result.data.every(sf => sf.origin_planet === 'PlanetX')).toBe(true);
  });
  ...
  it('filters by spacesuitColor', () => {
    const result = listSpacefarers({ planet: 'PlanetX', spacesuitColor: 'Silver' });
    expect(result.data.every(sf => sf.spacesuit_color === 'Silver')).toBe(true);
  });
  ...
  it('sorts by stardust_collection descending', () => {
    const result = listSpacefarers({ planet: 'PlanetX', sortBy: 'stardust_collection', sortDir: 'desc' });
