...
function findHandler() {
  const layer = (router.stack as Array<{ route?: { path: string; stack: Array<{ handle: Function }> } }>)
    .find(l => l.route?.path === '/login');
  return layer!.route!.stack[0].handle;
}

// Generate hash at runtime so it always matches the installed bcryptjs version
const HASHED_USER = {
...
describe('POST /login', () => {
  ...
  it('returns token and public user on valid credentials', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
      ...
      user: expect.objectContaining({ username: 'alice', role: 'admin', planet: 'PlanetX' }),
    }));
