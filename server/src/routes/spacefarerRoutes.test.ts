...
// ── Mock the mailer so no real SMTP is attempted ──────────────────────────
vi.mock('../mailer', () => ({ sendWelcomeEmail: vi.fn() }));

// ── Mock auth middleware — let all requests through as PlanetX admin ──────
...
const FAKE_SF: Spacefarer = {
  id: 'S1', name: 'Alice Starborn', email: 'alice@galactic.space',
  origin_planet: PLANET, spacesuit_color: 'Silver',
  stardust_collection: 500, wormhole_navigation_skill: 8,
  status: 'ACTIVE', launch_date: '2024-01-15T09:00:00Z',
  department_id: 'D1', position_id: 'P3',
  created_at: '2024-01-15T09:00:00Z', modified_at: '2024-01-15T09:00:00Z',
};

// ── Route handler tests ────────────────────────────────────────────────────

// We test the handler logic directly by finding the route layer
// (avoids spinning up a full HTTP server while still exercising real Express routing logic)
function findHandler(method: string, path: string) {
  const layer = (router.stack as Array<{ route?: { path: string; methods: Record<string, boolean>; stack: Array<{ handle: Function }> } }>)
    .find(l => l.route?.path === path && l.route.methods[method.toLowerCase()]);
...
describe('GET /spacefarers', () => {
  ...
  it('returns paginated list scoped to user planet', async () => {
    ...
    expect(db.listSpacefarers).toHaveBeenCalledWith(expect.objectContaining({ planet: PLANET }));
...
describe('GET /spacefarers/:id', () => {
  ...
  it('returns 404 when spacefarer not found', async () => {
    (db.getSpacefarer as MockedFunction<typeof db.getSpacefarer>).mockReturnValue(undefined);
  ...
  it('returns spacefarer when found', async () => {
    (db.getSpacefarer as MockedFunction<typeof db.getSpacefarer>).mockReturnValue(FAKE_SF);
...
describe('POST /spacefarers', () => {
  ...
  it('returns 400 when wormholeNavigationSkill is out of range', async () => {
    const req = authedReq({ body: { name: 'Test', email: 't@g.space', originPlanet: PLANET, wormholeNavigationSkill: 11 } });
    const res = mockRes();
  ...
  it('returns 400 when stardustCollection is negative', async () => {
    const req = authedReq({ body: { name: 'Test', email: 't@g.space', originPlanet: PLANET, stardustCollection: -1 } });
    const res = mockRes();
  ...
  it('creates spacefarer with defaults and triggers welcome email', async () => {
    ...
    (db.createSpacefarer as MockedFunction<typeof db.createSpacefarer>).mockReturnValue(created);

    const req = authedReq({ body: { name: 'Zara Nova', email: 'zara@g.space', originPlanet: PLANET } });
    const res = mockRes();
    ...
    expect(db.createSpacefarer).toHaveBeenCalledWith(
      expect.objectContaining({ stardustCollection: 100, wormholeNavigationSkill: 5 })
    );
...
describe('DELETE /spacefarers/:id', () => {
  ...
  it('returns 404 when spacefarer does not exist', async () => {
    (db.deleteSpacefarer as MockedFunction<typeof db.deleteSpacefarer>).mockReturnValue(false);
  ...
  it('returns 204 on successful delete', async () => {
    (db.deleteSpacefarer as MockedFunction<typeof db.deleteSpacefarer>).mockReturnValue(true);
...
describe('PATCH /spacefarers/:id', () => {
  ...
  it('returns 404 when spacefarer does not exist', async () => {
    ...
    const req = authedReq({ params: { id: 'S999' }, body: { spacesuitColor: 'Gold' } });
    const res = mockRes();
  ...
  it('returns 400 when wormholeNavigationSkill is out of range', async () => {
    ...
    const req = authedReq({ params: { id: 'S1' }, body: { wormholeNavigationSkill: 0 } });
    const res = mockRes();
  ...
  it('updates the spacefarer and returns the updated record', async () => {
    ...
    (db.getSpacefarer    as MockedFunction<typeof db.getSpacefarer>).mockReturnValue(FAKE_SF);
    (db.updateSpacefarer as MockedFunction<typeof db.updateSpacefarer>).mockReturnValue(updated);

    const req = authedReq({ params: { id: 'S1' }, body: { spacesuitColor: 'Gold' } });
    const res = mockRes();

    const handler = findHandler('patch', '/spacefarers/:id');
    await handler(req, res);

    expect(db.updateSpacefarer).toHaveBeenCalledWith('S1', PLANET, { spacesuitColor: 'Gold' });
...
describe('PATCH /spacefarers/:id/retire', () => {
  ...
  it('sets status to RETIRED', async () => {
    ...
    (db.getSpacefarer    as MockedFunction<typeof db.getSpacefarer>).mockReturnValue(FAKE_SF);
    (db.updateSpacefarer as MockedFunction<typeof db.updateSpacefarer>).mockReturnValue(retired);
    ...
    expect(db.updateSpacefarer).toHaveBeenCalledWith('S1', PLANET, { status: 'RETIRED' });
