...
import {
  listSpacefarers, getSpacefarer, createSpacefarer,
  updateSpacefarer, deleteSpacefarer,
...
// GET /api/spacefarers — row-filtered by JWT planet claim
router.get('/spacefarers', (req, res) => {
  const { status, spacesuitColor, sortBy, sortDir, page, pageSize } = req.query as Record<string, string>;
  const user = (req as unknown as AuthRequest).user;
  res.json(listSpacefarers({
    planet: user.planet,
...
// GET /api/spacefarers/:id
router.get('/spacefarers/:id', (req, res) => {
  const sf = getSpacefarer(req.params.id, (req as unknown as AuthRequest).user.planet);
  if (!sf) { res.status(404).json({ error: 'Not found' }); return; }
  res.json(sf);
});

// POST /api/spacefarers — admin only
router.post('/spacefarers', requireAdmin, async (req, res) => {
  ...
  if (!data.name || !data.email || !data.originPlanet) {
    res.status(400).json({ error: 'name, email and originPlanet are required' }); return;
  }
  const skill = Number(data.wormholeNavigationSkill);
  if (data.wormholeNavigationSkill !== undefined && (!Number.isInteger(skill) || skill < 1 || skill > 10)) {
    res.status(400).json({ error: 'wormholeNavigationSkill must be an integer between 1 and 10' }); return;
  }
  if (data.stardustCollection !== undefined && Number(data.stardustCollection) < 0) {
    res.status(400).json({ error: 'stardustCollection cannot be negative' }); return;
  }
...
router.patch('/spacefarers/:id', requireAdmin, (req, res) => {
  ...
  if (!getSpacefarer(req.params.id, planet)) { res.status(404).json({ error: 'Not found' }); return; }
  ...
  if (data.wormholeNavigationSkill !== undefined) {
    ...
    if (!Number.isInteger(s) || s < 1 || s > 10) {
      res.status(400).json({ error: 'wormholeNavigationSkill must be 1–10' }); return;
    }
  }
  res.json(updateSpacefarer(req.params.id, planet, data));
});

// DELETE /api/spacefarers/:id — admin only, planet-scoped
router.delete('/spacefarers/:id', requireAdmin, (req, res) => {
  const ok = deleteSpacefarer(req.params.id, (req as unknown as AuthRequest).user.planet);
  if (!ok) { res.status(404).json({ error: 'Not found' }); return; }
  res.status(204).end();
});

// PATCH /api/spacefarers/:id/retire — admin only
